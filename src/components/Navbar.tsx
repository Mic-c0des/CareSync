...
const linkClass = ({ isActive }: { isActive: boolean }) =>
  `px-3 py-2 rounded-full text-sm font-medium transition-colors whitespace-nowrap flex-shrink-0 ${
  ...
  return (
    <nav className="flex items-center justify-between border-b border-ink/10 px-4 py-3">
      ...
      <div className="flex items-center gap-1 whitespace-nowrap overflow-x-auto">
        ...
        <button
          onClick={() => signOut()}
          className="ml-2 px-3 py-2 rounded-full text-sm font-medium text-clay hover:bg-clay/10 whitespace-nowrap flex-shrink-0"
        >
          Sign out
        </button>
      </div>
